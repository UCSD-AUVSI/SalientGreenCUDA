...
void sg::LogGabor::release()
{
	...
	for (int i = 0; i < itsFilters.size(); i++)
		for (int j = 0; j < itsFilters.at(i).size(); j++)
			itsFilters.at(i).at(j).release();

	itsBufferReal.release();
	itsBufferImag.release();
	
	for (int i = 0; i < itsFFTBuffer.size(); i++)
		itsFFTBuffer.at(i).release();
}
...
void sg::LogGabor::addFilters( int numOrientations )
{
  ...
  cv::gpu::Stream stream;
	...
	for (int i = 0; i < itsFilters.size(); i++)
		itsFilters.at(i).resize( itsGaborScales.size() );

	// Allocate GPU filters and buffers
	for (int i = 0; i < itsFilters.size(); i++)
		for (int j = 0; j < itsFilters.at(i).size(); j++)
			itsFilters.at(i).at(j) = cv::gpu::GpuMat( itsHeight, itsWidth, CV_32FC2 );

	for (int i = 0; i < itsFFTBuffer.size(); i++)
		itsFFTBuffer.at(i) = cv::gpu::GpuMat( itsHeight, itsWidth, CV_32FC2 );
