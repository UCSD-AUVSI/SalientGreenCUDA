...
// if mat is complex, it only normalizes the real component. the complex component will get screwed up
void sg::normalizeGPU( cv::gpu::GpuMat & mat, float max )
{
  ...
  if (mat.type() == CV_32FC1)
    ...
    minMaxFC2Wrapper(mat, mi, ma);
  //printf("DEBUG size(%d,%d) min/max(%.4f, %.4f)\n", mat.size().width, mat.size().height, mi, ma);

  if( ma - mi == 0 )
	return;
  if (ma > 1e20 || ma < -1e20 || mi < -1e20 || mi > 1e20)
	return;
  ...
  if (mat.type() == CV_32FC1)
	...
	mulValueFC2Wrapper(mat, max / (ma - mi ), mat );
}
...
cv::gpu::GpuMat sg::padImageFFTGPU( cv::gpu::GpuMat const & input, cv::Size const & paddedSize )
{
  return padImageFFTGPU(input, cv::Size(), paddedSize);
  
  /*cv::gpu::GpuMat result;
  const int m = paddedSize.height;
  const int n = paddedSize.width;

  cv::gpu::copyMakeBorder( input, result, 0, m - input.rows, 0, n - input.cols, cv::BORDER_REPLICATE );

  cv::gpu::GpuMat planes[] = { result, cv::gpu::GpuMat::zeros( result.size(), CV_32F ) };

  cv::gpu::merge( planes, 2, result );
...
cv::gpu::GpuMat gpuZeros;
cv::gpu::GpuMat sg::padImageFFTGPU( cv::gpu::GpuMat const & input, cv::Size const & kernelSize, cv::Size const & paddedSize )
{
  ...
  cv::gpu::copyMakeBorder( input, result, 0, m - input.rows, 0, n - input.cols, cv::BORDER_WRAP  ); // was BORDER_REPLICATE

  if (gpuZeros.size().width != result.size().width || gpuZeros.size().height != result.size().height)
  {
	  gpuZeros = cv::gpu::GpuMat( result.size(), CV_32F);
	  gpuZeros.setTo(0.0);
  }

  std::vector<cv::gpu::GpuMat> channels;
  channels.push_back(result);
  channels.push_back(gpuZeros);
  cv::gpu::merge(channels, result);
  return result;
