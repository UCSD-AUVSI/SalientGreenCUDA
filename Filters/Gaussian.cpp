  ...
  itsIsValid( true ), itsBuffer( size, CV_32FC2 )
{
  ...
  itsFilter = cv::gpu::GpuMat( size, CV_32FC2 );
  ...
  double curSigma = initSigma;
	...
	cv::Mat totalFilter = cv::Mat( size, CV_32FC2 );
  ...
  stream.enqueueUpload( totalFilter, itsFilter );
...
void sg::DoGCenterSurround::operator()( cv::gpu::GpuMat const & input, cv::gpu::GpuMat & output )
{
	...
	cv::gpu::mulSpectrums( input, itsFilter, output, cv::DFT_COMPLEX_OUTPUT );
